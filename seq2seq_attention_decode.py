...
class BSDecoder(object):
    ...
    for b in self._batch_reader.data_iterator():
      ...
      for i in xrange(self._hps.batch_size):
        bs = beam_search.BeamSearch(
            ...
            self._vocab.get_word_id(data_manager.START_TOKEN),
            self._vocab.get_word_id(data_manager.END_TOKEN),
  ...
  def _DecodeBatch(self, article, abstract, output_ids):
    ...
    """
    decoded_output = ' '.join(self._vocab.get_words(output_ids))
    decoded_output = ', '.join([w for w in 
        decoded_output.split(data_manager.SEP_TOKEN) if w != ''])
    abstract = ', '.join(abstract.split(data_manager.SEP_TOKEN))
    end_p = decoded_output.find(data_manager.END_TOKEN, 0)
    if end_p != -1:
      decoded_output = decoded_output[:end_p]
    tf.logging.info('article:  %s', article)
    tf.logging.info('abstract: %s', abstract)
    tf.logging.info('decoded:  %s', decoded_output)
    self._decode_io.Write(abstract, decoded_output)
