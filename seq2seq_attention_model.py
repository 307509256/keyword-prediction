...
def _extract_argmax_and_embed(embedding, output_projection=None,
  ...
  def loop_function(prev, _):
    ...
    #return index of the word with the highest probability
...
class Seq2SeqAttentionModel(object):
  ...
  def run_train_step(self, sess, article_batch, abstract_batch, targets,
                     article_lens, abstract_lens, loss_weights, dropout):
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'), tf.device('/cpu:0'):
        ...
        # len(encode_inputs) * [batch_size, embed_size]
      ...
      with tf.variable_scope('decoder'), tf.device(self._next_device()):
        ...
        loop_function = None
        if hps.mode == 'decode':
          # [vsize, embed_size], ([num_hidden, vsize], [vsize])
        ...
        #len(encoder_outputs) * [batch_size, 1, 2 * num_hidden]
        encoder_outputs = [tf.reshape(x, [hps.batch_size, 1, 2*hps.num_hidden])
                           for x in encoder_outputs]
        #[batch_size, len(encoder_outputs), 2 * num_hidden]
